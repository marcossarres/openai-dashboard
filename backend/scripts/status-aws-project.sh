...
#
# Example:
#   STACK_NAME=cost-backend-formation ./scripts/status-aws-project.sh
...
if ${STACK_EXISTS}; then
  ...
  if [[ -z "${LOAD_BALANCER_ID}" ]]; then
    LOAD_BALANCER_ID=$(normalize_field "$(cf_resource_id ProjectLoadBalancer)")
  fi
  if [[ -z "${TARGET_GROUP_ID}" ]]; then
    TARGET_GROUP_ID=$(normalize_field "$(cf_resource_id SarrescostTargetGroup)")
  fi
  if [[ -z "${ASG_NAME}" ]]; then
    ASG_NAME=$(normalize_field "$(cf_resource_id ProjectAutoScalingGroup)")
  fi
...
report_ecs_service() {
  ...
  if [[ ${#service_names[@]} -eq 0 ]]; then
    service_names=("${ECS_SERVICE_NAME}")
  fi
  ...
  while (( start < ${#service_names[@]} )); do
    ...
    local response
    response=$(aws_cli ecs describe-services --cluster "${ECS_CLUSTER_NAME}" --services "${chunk[@]}" \
      --query 'services[*].[serviceName,status,desiredCount,runningCount,pendingCount,deployments[0].rolloutState]' \
      --output text 2>/dev/null || echo "")
    if [[ -z "${response}" ]]; then
      for svc in "${chunk[@]}"; do
        component_status "ECS" "ECS service (${svc})" "not created" "${svc} not found in cluster ${ECS_CLUSTER_NAME}." "MISSING"
      done
    else
...
report_ecs_task_definition() {
  local task_def
  if [[ -n "${ECS_CLUSTER_NAME}" && -n "${ECS_SERVICE_NAME}" ]]; then
    task_def=$(aws_cli ecs describe-services --cluster "${ECS_CLUSTER_NAME}" --services "${ECS_SERVICE_NAME}" \
      --query 'services[0].taskDefinition' --output text 2>/dev/null || echo "")
  fi

  if [[ -z "${task_def}" || "${task_def}" == "None" ]]; then
    task_def=$(normalize_field "$(cf_resource_id ProjectTaskDefinition)")
  fi

  if [[ -z "${task_def}" ]]; then
    component_status "ECS" "ECS task definition" "not created" "Task definition not found from ECS service or stack resources." "MISSING"
    return
  fi

  local info
  info=$(aws_cli ecs describe-task-definition --task-definition "${task_def}" \
    --query 'taskDefinition.[family,revision,status,networkMode,requiresCompatibilities[0]]' \
    --output text 2>/dev/null || echo "MISSING")

  if [[ "${info}" == "MISSING" || "${info}" == "None" ]]; then
    component_status "ECS" "ECS task definition (${task_def})" "not created" "Task definition not found." "MISSING"
    return
  fi

  IFS=$'\t' read -r family revision status network_mode compat <<<"${info}"
  local state
  state=$(classify_simple_state "${status}")
  component_status "ECS" "ECS task definition (${family}:${revision})" "${state}" "status=${status}, network_mode=${network_mode}, compat=${compat:-n/a}" "${status}"
}
...
report_launch_template() {
  local lt_id
  lt_id=$(normalize_field "$(cf_resource_id ProjectLaunchTemplate)")
  if [[ -z "${lt_id}" ]]; then
    component_status "EC2" "EC2 launch template" "not created" "Launch template not found in stack resources." "MISSING"
    return
  fi

  local info
  info=$(aws_cli ec2 describe-launch-templates --launch-template-ids "${lt_id}" \
    --query 'LaunchTemplates[0].[LaunchTemplateName,LatestVersionNumber,DefaultVersionNumber]' \
    --output text 2>/dev/null || echo "MISSING")

  if [[ "${info}" == "MISSING" || "${info}" == "None" ]]; then
    component_status "EC2" "EC2 launch template (${lt_id})" "not created" "Launch template not found." "MISSING"
    return
  fi

  IFS=$'\t' read -r name latest_version default_version <<<"${info}"
  component_status "EC2" "EC2 launch template (${name})" "working" "id=${lt_id}, latest_version=${latest_version}, default_version=${default_version}" "AVAILABLE"
}
...
report_execution_role() {
  local role_name
  role_name=$(normalize_field "$(cf_resource_id ProjectExecutionRole)")
  if [[ -z "${role_name}" ]]; then
    component_status "IAM" "IAM role (execution role)" "not created" "Execution role not found in stack resources." "MISSING"
    return
  fi

  local arn
  arn=$(aws_cli iam get-role --role-name "${role_name}" --query 'Role.Arn' --output text 2>/dev/null || echo "MISSING")
  if [[ "${arn}" == "MISSING" || "${arn}" == "None" ]]; then
    component_status "IAM" "IAM role (${role_name})" "not created" "Execution role not found in IAM." "MISSING"
    return
  fi

  component_status "IAM" "IAM role (${role_name})" "working" "execution role present, arn=${arn}" "AVAILABLE"
}

report_task_role() {
  local role_name
  role_name=$(normalize_field "$(cf_resource_id ProjectTaskRole)")
  if [[ -z "${role_name}" ]]; then
    component_status "IAM" "IAM role (task role)" "not created" "Task role not found in stack resources." "MISSING"
    return
  fi
  ...
  arn=$(aws_cli iam get-role --role-name "${role_name}" --query 'Role.Arn' --output text 2>/dev/null || echo "MISSING")
  if [[ "${arn}" == "MISSING" || "${arn}" == "None" ]]; then
    component_status "IAM" "IAM role (${role_name})" "not created" "Task role not found in IAM." "MISSING"
    return
  fi

  component_status "IAM" "IAM role (${role_name})" "working" "task role present, arn=${arn}" "AVAILABLE"
}

report_instance_profile() {
  local profile_name
  profile_name=$(normalize_field "$(cf_resource_id EcsInstanceProfile)")
  if [[ -z "${profile_name}" ]]; then
    component_status "IAM" "IAM instance profile" "not created" "Instance profile not found in stack resources." "MISSING"
    return
  fi

  local arn
  arn=$(aws_cli iam get-instance-profile --instance-profile-name "${profile_name}" --query 'InstanceProfile.Arn' --output text 2>/dev/null || echo "MISSING")
  if [[ "${arn}" == "MISSING" || "${arn}" == "None" ]]; then
    component_status "IAM" "IAM instance profile (${profile_name})" "not created" "Instance profile not found in IAM." "MISSING"
    return
  fi

  component_status "IAM" "IAM instance profile (${profile_name})" "working" "instance profile present, arn=${arn}" "AVAILABLE"
}
